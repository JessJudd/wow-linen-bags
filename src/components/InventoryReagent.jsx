...
export const InventoryReagent = ({
    ...
    let reagentOff = parent == 'menu' && inventoryCount > 0 ? `add-reagent-off` : '';
    
    return (
        <div className={`reagent ${reagent.name} parent-${parent} ${reagentOff}`}>
            <div className="reagent-main" 
            onClick={ parent == 'menu' && inventoryCount == 0 ? 
            ()=>setReagentCount(reagent.name, reagent.type, 1) : undefined 
                ...
                <figure className="reagent-icon-container">
                    ...
                    <img className={`reagent-icon ${inventoryCount > 0 ? "in-stock" : "" }`} src={imgUrl} />
                </figure>
                <span className="reagent-name">{`${reagent.type} ${reagent.name}`}</span>
            </div>
            {parent == 'inventory' && <div className="reagent-modifiers">
                <input 
                    id={type} 
